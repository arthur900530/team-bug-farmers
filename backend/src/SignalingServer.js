...
class SignalingServer {
                    ...
                    for (const receiver of allParticipants) {
                        if (receiver.userId !== userId) {
                            ...
                            if (receiverRtpCapabilities) {
                                try {
                                    const consumer = await this.mediasoupManager.createConsumer(receiver.userId, userId, receiverRtpCapabilities);
                                    if (consumer) {
     ...
     */
    relayAnswer(fromUserId, sdp, meetingId) {
        const recipients = this.meetingRegistry.listRecipients(meetingId, fromUserId);
        const answerMessage = {
            type: 'answer',
            meetingId,
     ...
     */
    relayIce(fromUserId, iceData, meetingId) {
        const recipients = this.meetingRegistry.listRecipients(meetingId, fromUserId);
        const iceMessage = {
            type: 'ice-candidate',
            meetingId,
     ...
     */
    notifyOthers(meetingId, excludeUserId, event) {
     ...
     */
    findClientWebSocket(userId) {
        for (const [ws, conn] of this.clients.entries()) {
            if (conn.userId === userId && ws.readyState === ws_1.WebSocket.OPEN) {
                return ws;
            }
        }
        return null;
     ...
     */
    createMediasoupAnswerSdp(transport, clientOfferSdp) {
     ...
     */
    extractRtpParametersFromSdp(sdp) {
        try {
     ...
     */
    extractRtpCapabilitiesFromSdp(sdp) {
        try {
            // Extract audio media section
            const audioSectionMatch = sdp.match(/m=audio\s+(\d+)\s+([^\r\n]+)\s+([^\r\n]+)/);
            if (!audioSectionMatch || !audioSectionMatch[3]) {
     ...
     */
    async createConsumersForUser(receiverUserId, meetingId) {
        ...
        for (const sender of otherParticipants) {
            ...
            if (producer) {
                try {
                    const consumer = await this.mediasoupManager.createConsumer(receiverUserId, sender.userId, receiverRtpCapabilities);
                    if (consumer) {
